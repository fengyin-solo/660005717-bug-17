...
import type { VolumeData, ROIResult, ROI, ROIShape, Plane } from '@/types'
...
function loadROIs(): ROI[] {
  try {
    ...
    return parsed.map((r: any) => ({
      ...
      shape: (['sphere', 'ellipsoid', 'box', 'polygon'].includes(r.shape) ? r.shape : 'sphere') as ROIShape,
      ...
      points: Array.isArray(r.points) ? r.points.map((p: any[]) => p.map(Number)) : undefined,
    }))
  } catch {
    return defaultROIs()
  }
...
export const useImagingStore = defineStore('imaging', () => {
  ...
  const rois = ref<ROI[]>(loadROIs())
  const roiResults = ref<ROIResult[]>([])
  const analyzeError = ref('')
  const activeTool = ref<ROIShape | 'select'>('select')
  const activePlane = ref<Plane | null>(null)
  const mprSlice = ref<Record<Plane, number>>({ axial: 32, coronal: 32, sagittal: 32 })
  const selectedROIId = ref<string | null>(null)

  const dims = computed<[number, number, number]>(() => volumeData.value?.dimensions ?? [64, 64, 64])

  function persist() {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(rois.value)) } catch { /* 忽略存储失败 */ }
  }
  ...
  // ---- ROI 增删改 ----
  function addROI(roi?: Partial<ROI>) {
  ...
  function removeROI(id: string) {
    ...
    if (selectedROIId.value === id) selectedROIId.value = null
    persist()
    scheduleAnalyze()
  }

  function updateROI(id: string, patch: Partial<ROI>) {
  ...
  function setTool(tool: ROIShape | 'select', plane?: Plane) {
    activeTool.value = tool
    activePlane.value = plane ?? null
  }

  /** 本地参数校验：返回错误原因；null 表示可提交测量 */
  function validateROI(roi: ROI): string | null {
    const [d, h, w] = dims.value
    const [cx, cy, cz] = roi.center.map(v => Number(v))
    if (![cx, cy, cz].every(v => Number.isFinite(v))) return '中心坐标不是有效数字'
    ...
    if (roi.shape === 'polygon') {
      ...
      if (pts.length < 3) return `多边形至少需要 3 个顶点（当前 ${pts.length} 个），继续在影像上单击添加顶点`
      ...
      if (!inPlane) return '多边形顶点超出影像范围'
      const n = roi.plane === 'axial' ? d : roi.plane === 'coronal' ? h : w
      if (roi.slice == null || roi.slice < 0 || roi.slice >= n) return `多边形所在切片超出影像范围（0~${n - 1}）`
      return null
    }

    if (roi.shape === 'box') {
      const s = roi.size ?? []
      if (s.length !== 3 || !s.every(v => Number(v) > 0 && Number.isFinite(Number(v)))) return '矩形长宽高必须均为正数'
      if (centerOutOfBounds) return `标记中心超出影像范围（允许范围 x:0~${w - 1} y:0~${h - 1} z:0~${d - 1}），范围内没有可测量体素`
      return null
    }

    if (radiusInvalid) return '半径必须为正数'
    if (radiiInvalid) return '椭圆三轴半径必须均为正数'
    if (centerOutOfBounds) return `标记中心超出影像范围（允许范围 x:0~${w - 1} y:0~${h - 1} z:0~${d - 1}），范围内没有可测量体素`
    return null
  }
  ...
  async function analyzeROIs() {
    ...
    try {
      ...
      if (seq !== analyzeSeq) return // 已有更新的测量请求，丢弃本次过期结果
      const byId = new Map<string, ROIResult>((data.rois as ROIResult[]).map(r => [String(r.id), r]))
      ...
      if (seq === analyzeSeq) loading.value = false
    }
  }

  function emptyResult(r: ROI, reason: string): ROIResult {
    return {
